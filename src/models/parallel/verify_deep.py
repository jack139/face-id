...
from models.deepface import verify as verify_deep
...
def get_features_b64_thread(face_algorithm, b64_data):
    ...
    if face_algorithm=='vgg':
        ...
        encoding_list, face_boxes = verify_deep.get_features_b64(b64_data)
...
def is_match_b64(b64_data1, b64_data2):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_list = [
            ...
            executor.submit(get_features_b64_thread, 'deep', b64_data1), # 1
            executor.submit(get_features_b64_thread, 'vgg', b64_data2), # 2
            executor.submit(get_features_b64_thread, 'deep', b64_data2), # 3
        ]
    ...
    distance_deep = face_distance([results[1][0][0]], results[3][0][0])
    if distance_deep <= ALGORITHM['deep']['distance_threshold']:
        return True, distance_deep/ALGORITHM['deep']['distance_threshold']
...
def get_features_b64(b64_data):
    ...
    encoding_list2, face_boxes2 = verify_deep.get_features_b64(b64_data)

    if len(face_boxes1) == 0:
        return [], []
    else:
        # 返回4个，第2个位置留给evo，第3个位置留给rec，保持一直， 2020-06-25
...
def is_match_b64_serial(b64_data1, b64_data2):
    ...
    distance_deep = face_distance([encoding_list1[1]], encoding_list2[1])
    if distance_deep <= ALGORITHM['deep']['distance_threshold']:
        return True, distance_deep
...
def is_match_b64_2(encoding_list_db, b64_data):
    encoding_list1 = [[], []]  # [ vgg, deep ]
    for i in range(len(encoding_list_db)):
        encoding_list1[0].append(encoding_list_db[i][0]) # vgg index=0
        encoding_list1[1].append(encoding_list_db[i][2]) # deep index=2  2020-06-23
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_list = [
            ...
            executor.submit(get_features_b64_thread, 'deep', b64_data), # 1
        ]
        for future in concurrent.futures.as_completed(future_list):
            pos = future_list.index(future)
            results[pos] = future.result()
    ...
    distance_deep = face_distance(encoding_list1[1], results[1][0][0])
    x = distance_deep <= ALGORITHM['deep']['distance_threshold']
    if x.any():
        return True, distance_deep/ALGORITHM['deep']['distance_threshold']
